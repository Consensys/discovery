...
public class SanityTestVectors {
  ...
  void testOrdinaryPingPacket() {
    ...
    Bytes12 nonce = Bytes12
        .fromHexString("0xffffffffffffffffffffffff");
    Bytes16 readKey = Bytes16
        .fromHexString("0x00000000000000000000000000000000");
    Bytes16 maskingIV = Bytes16
        .fromHexString("0x00000000000000000000000000000000");
    Bytes pingReqId = Bytes
        .fromHexString("0x00000001");
    ...
    OrdinaryMessagePacket packet = OrdinaryMessagePacket
        .create(maskingIV, header, pingMessage, readKey);
    Bytes16 maskingKey = Bytes16.wrap(destNodeId, 0);
    RawPacket rawPacket =
        RawPacket.createAndMask(maskingIV, packet, maskingKey);
    ...
    V5Message v5Message = messagePacket
        .decryptMessage(maskingIV, readKey, NodeRecordFactory.DEFAULT);
  ...
  void testWhoAreYouPacket() {
    ...
    Bytes16 maskingIV = Bytes16
        .fromHexString("0x00000000000000000000000000000000");
    Bytes whoareyouChallengeData = Bytes
        .fromHexString("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000000");
    Bytes12 whoareyouRequestNonce = Bytes12
        .fromHexString("0x0102030405060708090a0b0c");
    Bytes16 whoareyouIdNonce = Bytes16
        .fromHexString("0x0102030405060708090a0b0c0d0e0f10");
  ...
  void testHandshakePacket() {
    ...
    Bytes12 nonce = Bytes12
        .fromHexString("0xffffffffffffffffffffffff");
    Bytes16 readKey = Bytes16
        .fromHexString("0x4f9fac6de7567d1e3b1241dffe90f662");
    Bytes pingReqId = Bytes
        .fromHexString("0x00000001");
    ...
    // handshake inputs:
    Bytes whoareyouChallengeData = Bytes
        .fromHexString("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000001");
    Bytes12 whoareyouRequestNonce = Bytes12
        .fromHexString("0x0102030405060708090a0b0c");
    Bytes16 whoareyouIdNonce = Bytes16
        .fromHexString("0x0102030405060708090a0b0c0d0e0f10");
    UInt64 whoareyouEnrSeq = UInt64.valueOf(1);
    Bytes ephemeralKey = Bytes
        .fromHexString("0x0288ef00023598499cb6c940146d050d2b1fb914198c327f76aad590bead68b6");
    Bytes ephemeralPubkey = Bytes
        .fromHexString("0x039a003ba6517b473fa0cd74aefe99dadfdb34627f90fec6362df85803908f53a5");
    ...
    PingMessage pingMessage = new PingMessage(pingReqId, pingEnrSeq);
    Bytes idSignature = HandshakeAuthData
        .signId(whoareyouChallengeData, ephemeralPubkey, destNodeId, nodeAKey);

    Header<HandshakeAuthData> header = Header
        .createHandshakeHeader(srcNodeId, nonce, idSignature, ephemeralPubkey, Optional.empty());
  ...
  void testHandshakeWithEnrPacket() {
    ...
    Bytes12 nonce = Bytes12
        .fromHexString("0xffffffffffffffffffffffff");
    Bytes16 readKey = Bytes16
        .fromHexString("0x53b1c075f41876423154e157470c2f48");
    Bytes pingReqId = Bytes
        .fromHexString("0x00000001");
    UInt64 pingEnrSeq = UInt64.valueOf(1);

    NodeRecord nodeRecord = NodeRecord.fromValues(IdentitySchemaInterpreter.V4, UInt64.valueOf(1),
        List.of(new EnrField(EnrField.PKEY_SECP256K1, Bytes
                .fromHexString("0x0313D14211E0287B2361A1615890A9B5212080546D0A257AE4CFF96CF534992CB9")),
            new EnrField(EnrField.ID, IdentitySchema.V4),
            new EnrField(EnrField.IP_V4, Bytes.fromHexString("0x7F000001"))
        ));
    nodeRecord.setSignature(Bytes.fromHexString(
    ...
    System.out.println(nodeRecord.asBase64());
    ...
    Header<HandshakeAuthData> header = Header
        .createHandshakeHeader(srcNodeId, nonce, idSignature, ephemeralPubkey,
    ...
    Header<HandshakeAuthData> header1 = packet1.getHeader();
    HandshakeAuthData authData1 = header1.getAuthData();
    Optional<NodeRecord> record = authData1.getNodeRecord(NodeRecordFactory.DEFAULT);
    assertThat(authData1.getBytes()).isEqualTo(header.getAuthData().getBytes());
    assertThat(isFieldsEqual(authData1, header.getAuthData(), NodeRecordFactory.DEFAULT)).isTrue();
    assertThat(isFieldsEqual(header1.getStaticHeader(), header.getStaticHeader())).isTrue();
  }
